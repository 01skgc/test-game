(function () {
    ...
    var canvas; var ctx; var img = {snow:null, snow_man:null}; var snow_x = 0; var snow_y = 0; var snow_man_x = 0; var snow_man_y = 0; var requestId = 0;
...
function setHandlers() {
    ...
    function loadAssets() {
         ...
         //アニメーションの開始
         canvas.addEventListener('click', function (){ if (!requestId) {renderFrame();} });
         ...
         //中央の Left 位置を求める関数
         function getCenterPosition(containerWidth, itemWidth) { return (containerWidth / 2) - (itemWidth / 2); }
...
audioPlayed = false;
    ...
    //あたり判定処理
    if (isHit(img.snow, snow_x, snow_y, img.snow_man, snow_man_x, snow_man_y)) { hitJob(); }
...
function isHit(a, ax, ay, b, bx, by) {
    return (((ax <= bx && bx <= ax + a.width) || (bx <= ax && ax <= bx + b.width)) && ((ay <= by && by <= ay + a.height) || (by <= ay && ay <= by + b.height))); }

//あたり判定の処理
function hitJob() {
    ctx.font = 'bold 20px sans-serif'; ctx.fillStyle = 'red'; ctx.fillText('Hit!!', 5, 24);
